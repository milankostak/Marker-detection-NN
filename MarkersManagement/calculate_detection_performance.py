...
def get_metrics(values: np.ndarray, name: str, limit: float, reverse: bool = False):
    ...
    print(f"{name} bad count:", values.shape[0] - values_good.shape[0])
...
def main():
    base_path = "D:/images/draw3/original/"

    #  with open(base_path + "test_onlyGoodHue.txt") as file:
    ...
    for i in range(count):
        ...
        if len(detected_values) == 0:
            ...
            detected_content = get_content2(detected_values[2], detected_values[3], detected_values[4],
                                            detected_values[5])
